...
public class Actions {
	...
	@Test
	public void TestRememberLogin() throws InterruptedException, IOException {
		Mozila.findElement(By.xpath("/html/body/div[2]/nav/div/div[2]/ul/li[1]/a[2]/span[2]")).click();
		Thread.sleep(1000);
		log.info("Click on Home button");
		...
		Mozila.findElement(By.xpath("//*[@id=\"password\"]")).clear();
		log.info("Cleared the Password text box");
		Mozila.findElement(By.xpath("//*[@id=\"username\"]")).sendKeys("admin");
		log.info("Username entered in the Username text box");
		Mozila.findElement(By.xpath("//*[@id=\"password\"]")).sendKeys("admin");
		log.info("Password entered in the Password text box");
		...
		//get cookies
		Set<Cookie> All = Mozila.manage().getCookies();
		Mozila.close();
		FirefoxDriver Mozila = new FirefoxDriver();
		Mozila.manage().window().maximize();;
		Mozila.navigate().to("http://192.168.178.227:8080/");
		...
		for (Cookie cookie : All) {
			Mozila.manage().addCookie(cookie);
		}
		Mozila.navigate().refresh();
		Thread.sleep(2000);
		Assert.assertTrue(Mozila.findElement(By.xpath("/html/body/div[3]/div[1]/div/div/div[2]/div/div")).isDisplayed(), "User is no longer logged in! Remember me doesn't work");
		Reporter.log("User is still logged in. Remember me works");
